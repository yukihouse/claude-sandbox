#!/usr/bin/env bash
# Build a standalone Linux executable for the Tetris demo game.
#
# Produces a single-file binary (no Python install required to run it) plus
# a zip archive ready for distribution.
#
# Requirements: Linux, and uv (https://docs.astral.sh/uv/).
#
# Usage:
...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "error: this script must be run on Linux (PyInstaller builds a native binary for the host OS)." >&2
  exit 1
fi
...
ARCHIVE_NAME="tetris-${VERSION}-linux-${ARCH}.zip"
