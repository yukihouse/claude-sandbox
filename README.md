...
カウンターに加えて、Rustの `checked_add` によるオーバーフロー安全性を体感できる
「🛡️ 壊れないカウンター」機能つきです。u8の上限（255）付近までジャンプしてから
+1を押すと、255を超えようとした時点でRustが安全に停止し、他言語にありがちな
「値が0に巻き戻る」ような挙動にはならないことを確認できます。
...
カウンターに加えて、「🔬 中身が見えるカウンター」機能つきです。今のカウントの値を
u8/u16/u32のどの型で見るか選ぶと、そのバイト列（16進数・2進数）と、その型に
収まりきるかどうかをZigが実際に計算して表示します。
